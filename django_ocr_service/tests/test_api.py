...
class TestGetTokenAPI:
    ...
    def setup_method(self):
        ...
        :return:
        """
        self.django_client, self.user, self.token_true = create_rest_user_login_generate_token()
    ...
    def test_get_token_authenticated(self):
        ...
        response = self.django_client.get('/api/get-token/', content_type='application/json')
        expected_token = ExpiringToken.objects.get(user=self.user).key
        assert response.status_code == 200 and response.data == {'token': expected_token}
...
class TestPostOCR:
    ...
    def test_post_ocr_authenticated_from_uploaded_path(self):
        ...
        """
        self.django_client.force_authenticate(user=self.user)
        token_response = self.django_client.get('/api/get-token/', content_type='application/json')
        ...
        assert len(ocrinput_objs) > 0 and response.status_code == 200 and isinstance(response.data["response"], dict)
    ...
    def test_post_ocr_authenticated_direct_upload(self):
        ...
        upload_file = SimpleUploadedFile(name=filename,
                                         content=data.read(),
